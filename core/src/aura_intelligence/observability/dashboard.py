...
class UnifiedDashboard:
    ...
    def update_tda_metrics(self, algorithm: str, anomaly_score: float, duration: float):
        """Update TDA-related metrics"""
        self.tda_computations.labels(algorithm=algorithm, status="success").inc()
    ...
    async def _should_fire_alert(self, alert: Alert) -> bool:
        ...
        if alert.name in self._alert_history:
            ...
            if (datetime.now(timezone.utc) - last_fired).seconds < alert.cooldown:
                return False
    ...
    async def _fire_alert(self, alert: Alert):
        ...
        # Record alert
        self._alert_history[alert.name] = datetime.now(timezone.utc)
